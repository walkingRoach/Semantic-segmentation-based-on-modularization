...
class ShuffleNetV2(nn.Module):
    ...
    def __init__(self, remove_fc=False, scale=1.0, in_channels=3, c_tag=0.5, num_classes=1000, activation=nn.ReLU,
...
def shuffleNet(pretrained=False, remove_fc=True, root=os.path.expanduser("~/.cache/torch/checkpoints/"), **kwargs):
    model = ShuffleNetV2(remove_fc=remove_fc, **kwargs)
    if pretrained:
        model.load_state_dict(load_from_path('shuffleNet', root, remove_fc))
    return model


def load_from_path(net_name, model_dir="~/.cache/torch/checkpoints/", remove_fc=True, map_location='cuda:0'):
    cached_file = os.path.join(model_dir, net_name + '.pth.tar')
    if not os.path.isfile(cached_file):
        raise print('can not load checkpoints file')
    print("=> loading pretrained weight '{}' ".format(cached_file))
    checkpoints = torch.load(cached_file, map_location=map_location)['state_dict']
    target_state = OrderedDict()
    for k, v in checkpoints.items():
        # if k[:7] == 'module.' and remove_fc:
        k = k[7:]
        # target_state[k] = v
        if remove_fc and ('fc' in k):
            continue
        else:
            target_state[k] = v
